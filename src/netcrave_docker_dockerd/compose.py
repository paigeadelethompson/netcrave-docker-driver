...
def netcrave_compose():
    if not Path.exists(compose_from_config_directory()):
        templ_path = default_compose()
        try:
            cfg = ConfigFile.from_filename("docker-compose.yml")
            for index in cfg.config.get("services"):
                if cfg.config.get("services").get(index).get("build") != None:
                    if cfg.config.get("services").get(index).get("build").get("args") != None:
                        if cfg.config.get("services").get(index).get("build").get("args").get("TEMPLATE_ROOT") != None:
                            cfg.config["services"][index]["build"]["args"]["TEMPLATE_ROOT"] = str(Path.joinpath(
                                templ_path.parent,
                                cfg.config.get("services").get(index).get("build").get("args").get("TEMPLATE_ROOT")))
                        if cfg.config.get("services").get(index).get("build").get("dockerfile") != None:
