...
class installer():
    def _tarfile_mapper(self, archive,
                        path_formatter=lambda
                        path: "/opt/netcrave/{path}".format(path=path),
                        exclude_file=lambda path: False):
        return [{"src": index, "dst": path_formatter(index)} for index in tarfile.open(
            archive).getnames() if not exclude_file(index)]

    def __init__(self):
        self._packages = [{"x86_64": "https://github.com/kata-containers/kata-containers/releases/download/3.2.0/kata-static-3.2.0-amd64.tar.xz",
                           "map": lambda archive: self._tarfile_mapper(archive,
                                                                       lambda path: (path.replace("./opt/kata",
                                                                                                  "/opt/netcrave")),
                                                                       lambda path: (len([index for index in ("versions.yaml",
                          ...
                          {"x86_64": "https://download.docker.com/linux/static/stable/x86_64/docker-24.0.7.tgz",
                           "map": lambda archive: self._tarfile_mapper(archive,
                                                                       lambda path: ("/opt/netcrave/bin/{path}".format(path="/".join(path.split("/")[1::]))))},
                          {"x86_64": "https://download.docker.com/linux/static/stable/x86_64/docker-rootless-extras-24.0.7.tgz",
                           "map": lambda archive: self._tarfile_mapper(archive,
                                                                       lambda path: (path.replace("docker-rootless-extras",
                                                                                                  "/opt/netcrave/bin")))},
                          ...
                          {"name": "Firecracker",
                           ...
                           "map": (lambda archive: self._tarfile_mapper(archive,
                                                                        lambda path: (path.replace("release-v1.5.1-x86_64",
                                                                                                   "/opt/netcrave/bin")),
                                                                        lambda path: (len([index for index in ("CHANGES",
                                                                                                               "THIRD-PARTY",
                                                                                                               "SHA256SUMS",
                                                                                                               "NOTICE",
                                                                                                               "LICENSE",
                          ...
                          {"name": "CRI-O",
                           ...
                           "map": lambda archive: self._tarfile_mapper(archive,
                                                                       lambda path: ("/opt/netcrave/{path}".format(path="/".join(path.split("/")[1::]))),
                                                                       lambda path: (len([index for index in ("etc",
                                                                                                              "crictl.yaml",
                                                                                                              "crio.conf",
                                                                                                              "crio-umount.conf",
                                                                                                              "README.md",
                    ...
                    if mapped.get("src") == member:
                        ...
                        if not file.isdir():
                            async with aiofiles.open(mapped.get("dst"), "wb") as out:
                                ...
                                log.debug(
                                    "extracted file {filename}".format(
                                        filename=mapped.get("dst")))
                            async with aiofiles.open(mapped.get("dst"), "rb") as check:
                                ...
                                if (mapped.get("dst").startswith("/opt/netcrave/cni-plugins")
                                    ...
                                    log.debug(
                                        "set executable {filename}".format(
                                            filename=mapped.get("dst")))
                            ...
                            log.debug(
                                "created directory {direc}".format(
                                    direc=mapped.get("dst")))

        if index.get("dst") is None:
            log.debug("deleting {tempfile}".format(tempfile=dl))
            Path(dl).unlink()

    async def install_one(self, index):
        log = logging.getLogger(__name__)
        if index.get("dst") is not None and not Path(
                index.get("dst")).parent.exists():
            Path.mkdir(
                Path(
                    index.get("dst")).parent,
                parents=True,
                exist_ok=True)
            log.debug(
                "created directory {direc}".format(
        ...
        dl = index.get("dst") is not None and index.get("dst") or mktemp()
